...
Private Sub ImportSourceDataToWorksheet(ByVal sourceSheet As Worksheet, ByVal targetSheet As Worksheet)
    ...
    If lastRow < 2 Then
        Err.Raise vbObjectError + 1010, "ImportSourceDataToWorksheet", "The selected worksheet does not contain any data rows."
    End If
    sourceData = sourceSheet.Range(sourceSheet.Cells(1, 1), sourceSheet.Cells(lastRow, lastCol)).Value
    Set columnMap = DetectImportedColumnMap(sourceData)
    targetSheet.Cells.Clear
    WriteImportedHeaders targetSheet
    targetRow = 2
    For rowIndex = 2 To UBound(sourceData, 1)
        personalNumber = GetMappedImportedValue(sourceData, rowIndex, CLng(columnMap("personal_number")))
        fullName = BuildImportedFullName(sourceData, rowIndex, columnMap)
        birthDate = GetFormattedImportedBirthDate(sourceData, rowIndex, CLng(columnMap("birth_date")))
        militaryUnit = NormalizeUnitValue(GetMappedImportedValue(sourceData, rowIndex, CLng(columnMap("military_unit"))))
        If Len(personalNumber) > 0 Or Len(fullName) > 0 Or Len(birthDate) > 0 Or Len(militaryUnit) > 0 Then
            targetSheet.Cells(targetRow, IMPORTED_COL_SOURCE_ROW).Value = rowIndex
            targetSheet.Cells(targetRow, IMPORTED_COL_PERSONAL_NUMBER).Value = personalNumber
            targetSheet.Cells(targetRow, IMPORTED_COL_FULL_NAME).Value = fullName
            targetSheet.Cells(targetRow, IMPORTED_COL_BIRTH_DATE).Value = birthDate
            targetSheet.Cells(targetRow, IMPORTED_COL_MILITARY_UNIT).Value = militaryUnit
...
Private Sub ValidateImportedColumnMap(ByVal columnMap As Object)
    ...
    If Len(missingFields) > 0 Then
        Err.Raise vbObjectError + 1011, "DetectImportedColumnMap", _
            "Automatic column mapping failed. Missing required headers:" & vbCrLf & missingFields
    End If
End Sub
...
Private Function IsPersonalNumberHeader(ByVal headerText As String) As Boolean
    IsPersonalNumberHeader = HeaderContainsAny(headerText, Array( _
        "личный номер", _
        "табельный номер", _
        "табельный", _
...
Private Function IsFullNameHeader(ByVal headerText As String) As Boolean
    IsFullNameHeader = HeaderContainsAny(headerText, Array( _
        "фио", _
        "ф.и.о", _
        "полное имя", _
        "full name"))
...
Private Function IsBirthDateHeader(ByVal headerText As String) As Boolean
    IsBirthDateHeader = HeaderContainsAny(headerText, Array( _
        "дата рождения", _
        "рождения", _
        "birth date", _
        "date of birth"))
...
Private Function IsMilitaryUnitHeader(ByVal headerText As String) As Boolean
    IsMilitaryUnitHeader = HeaderContainsAny(headerText, Array( _
        "войсковая часть", _
        "в ч", _
        "вч", _
        "в/ч", _
        "подразделение", _
        "наименование подразделения", _
        "место службы", _
        "military unit", _
        "department"))
End Function
